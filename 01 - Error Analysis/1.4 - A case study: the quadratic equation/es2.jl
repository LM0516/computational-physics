...
p3 = plot_generic(x_test, f_naive, xscale=:log10, label="Naive", 
          xlabel=L"x", ylabel=L"f(x)", title="Comparison of Algorithms")
plot_add!(p3, x_test, f_mclaurin_test, label="Maclaurin (n=$n)", marker=:square)
# Add the expm1 truth as a dashed line
plot_add!(p3, x_test, f_true_test, label="expm1 (Truth)", linestyle=:dash, legend=:bottomright)

# === Show the results ===
p = plot(p1, p2, p3, layout=(1, 3), size=(1200, 400))
save_graph(p, "series_comparison", "1-4")
display(p)
readline()
