...
function plot_interpolation(n_values::Array{Int}, a::Int, b::Int, f::Function, function_name)
    ...
    for (i, n) in enumerate(n_values)
        ...
        p_interp = [barycentric_lagrange(x, x_nodes, f, method=lag) for x in x_eval_range]
    ...
    function_name == "|x|" ? save_plot(p, "abs_x-plot", "3-2") : save_plot(p, "$function_name-plot", "3-2")
    display(p)
    readline()
end
