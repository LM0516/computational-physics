...
% TODO: Sistemare la disposizione delle figure, sono troppe e troppo vicine.
\begin{solution}
	...
	\paragraph{Discussion}
	\begin{itemize}
		\item \textbf{Equidistant Nodes \& Runge's Phenomenon:} The uniformly spaced
		...
		\item \textbf{Chebyshev Nodes \& Spectral Convergence:} Deploying Chebyshev
...
Solving the polynomial interpolation problem by setting up a general $n \times
	n$ linear system, such as using the power basis with a \textbf{Vandermonde
...
\textbf{LU} or \textbf{QR} scales as $\propto n^3$.

It is crucial to distinguish between \textbf{interpolation} ($m=n$) and
\textbf{least squares} ($m > n$). While QR decomposition is slightly more
...
The \textbf{barycentric form} of Lagrange interpolation overcomes these
...
\textbf{updating} the interpolant; adding a new point $x_{k}$ costs only
...
\textbf{equidistant} or \textbf{Chebyshev points}, analytic formulas allow the
...
\begin{itemize}
	\item \textbf{Improved Conditioning:} Chebyshev nodes significantly improve
	...
	\item \textbf{Numerical Stability:} When paired with the barycentric form,
	...
	\item \textbf{Avoidance of Precision Loss:} Unlike equidistant nodes, which
