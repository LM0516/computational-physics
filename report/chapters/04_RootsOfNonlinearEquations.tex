...
\begin{solution}
	...
	\textit{maximum bound} of the error decreases monotonically by exactly $1/2$
	at each step, the \textit{actual} error $\epsilon_n = |x_n - r|$ does not
...
\begin{solution}
	...
	\begin{itemize}
		\item \textbf{Functions 1 and 2:} Both exhibit a convergence rate of $q
		      ...
		      instability.
		\item \textbf{Function 3:} The convergence rate degenerates to $q
...
\begin{solution}
	...
	\begin{center}
		...
		\captionof{table}{Newton's method results for varying initial guesses.
			"Overshoot" denotes instances where the algorithm converged to a root
			other than the one geometrically closest to the initial guess.}
	...
	\begin{itemize}
		\item \textbf{Case $x_1 = 2.0$:} The function has a local minimum
		      ...
		      and landing directly into the basin of attraction for the root at $r
			      \approx 6.31$.
		\item \textbf{Case $x_1 = 5.0$:} Similarly, $x=5$ sits near a local
...
\begin{solution}
	...
	utilised as the initial guesses, $x_0$ and $x_1$.
	...
	employed. \textbf{Algorithm 1} utilises a linear least-squares fit on the
	logarithmic errors, whereas \textbf{Algorithm 2} computes the local
	...
	\begin{center}
		...
		\begin{tabular}{l c S[table-format=2.6] S[table-format=2.3] c}
			\toprule
			\textbf{Function} & \textbf{Interval} & {\textbf{Root} ($x^*$)} & {$q$ \textbf{(Alg 1)}} & {$q$ \textbf{(Alg 2)}} \\
			\midrule
	...
	\begin{itemize}
		...
		\item For $f_3(x)$, the algorithm drops to linear convergence ($q
		      ...
		      derivative $f_3'(x) = e^{x+1} - 1$ at $x = -1$ yields $0$. Therefore, $x
			      = -1$ is a \textit{multiple root} (multiplicity $m=2$), and root-finding
	...
	\textit{catastrophic cancellation}, severely amplifying numerical noise.
...
\begin{solution}
	...
	\begin{center}
		...
		\begin{tabular}{l c S[table-format=2.6] S[table-format=1.3] S[table-format=1.3]}
			\toprule
			\textbf{Function} & \textbf{Initial Estimates} & {\textbf{Root} ($x^*$)} & {$q$} & {$C$} \\
			\midrule
	...
	technique (\textbf{Algorithm 1}) was used. The successive differences
	approach (\textbf{Algorithm 2}) consistently failed for this exercise,
