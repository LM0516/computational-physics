...
function solutions(a::Real, b::Real, f::Function, nodes, label, exact; rule::String="trapezoidal")
    ...
    if rule == "trapezoidal"
        for j in 1:length(nodes)
            s[j] = trapezoidal_rule(a, b, f, nodes=nodes[j])
        end
    elseif rule == "simpson"
        for j in 1:length(nodes)
            s[j] = simpson_rule(a, b, f, nodes=nodes[j])
        end
    else
...
function errors(s::Vector{Float64}, exact::Float64)
    ...
    relative_plots = plot(relative_errors, label="Relative errors", xscale=:log, yscale=:log)
    xlabel!("Nodes")
    ylabel!("Relative error")

    # Plot absolute errors
    absolute_plots = plot(absolute_errors, label="Absolute errors", xscale=:log, yscale=:log)
    xlabel!("Nodes")
