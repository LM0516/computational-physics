...
function main()
    ...
    # plotta i valori
    p = plot_generic(0:N, xs_f32, label="Float32", linewidth=1, xlabel=L"k", ylabel=L"x_k")
    plot_add!(p, 0:N, xs_f64, label="Float64", linewidth=1)
    plot_add!(p, 0:N, xs_b, label="BigFloat", linewidth=1)
    plot_add!(p, 0:N, float.(xs_r), label="Rational (exact)", linewidth=1)
    hline!(p, [6], label="convergence", linestyle=:dash, color=:black)
