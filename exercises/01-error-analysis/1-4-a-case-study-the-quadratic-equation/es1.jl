...
function generate_stability(offsets, datasets)
    ...
    err_dp64 = [clamp_err(abs(var_double_pass(Float64.(d)) - true_var) / true_var) for d in datasets]
...
function main()
    ...
    println("\n")
    println("="^60)
    println("Double-pass vs Single-pass comparison")
    ...
    for (c, d) in zip(offsets, datasets)
        ...
        println("Single-pass 32-bit: ", var_single_pass(d32))
        println("Single-pass 64-bit: ", var_single_pass(d64))
    end
