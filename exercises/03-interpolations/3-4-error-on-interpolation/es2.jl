...
function plot_interpolation(n_values::Array{Int}, a::Int, b::Int, f::Function, function_name; interp_type=cheb, fig_name="plot")
    ...
    println("Function: $function_name | Type: $title_str | K ≈ $K (slope ≈ $slope)")
    ...
    # FIX: Change the reference on the report
    save_plot(p, "$fig_name-$title_str", "3-4")
    display(p)
    readline()
end
...
function main()
    ...
    plot_interpolation(n, a, b, f_b, L"\tanh(5x + 2)", interp_type=cheb, fig_name="fb")
    plot_interpolation(n, a, b, f_c, L"\cosh(\sin(x))", interp_type=cheb, fig_name="fc")
